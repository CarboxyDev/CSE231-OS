...
/*
 *  This program uses mutex locks for the Phil Dining problem variation which has 2 bowls
*/

pthread_mutex_t forks[5];
pthread_mutex_t sauce_bowls[2];
pthread_t phil[5];
...
void eat(int x) {
    ...
    pthread_mutex_lock(&sauce_bowls[x % 2]);
    ...
    pthread_mutex_unlock(&sauce_bowls[x % 2]);
...
int main(){
    while (1 == 1) {
        ...
        for (int i=0; i < 2; i++) {
            int tryInit = pthread_mutex_init(&sauce_bowls[i], NULL);
            if (tryInit == -1) {
                printf("[!] Unable to use mutex\n");
                return 0;
            }
        ...
        for (int i=0; i < 2; i++) {
            int tryDestroy = pthread_mutex_destroy(&sauce_bowls[i]);
            if (tryDestroy != 0) {
                return 0;
            }
        }
